...
{
    public class FastFlares : StoryboardObjectGenerator
    {
        [Configurable]
        ...
        public override void Generate()
        {
            ...
            }))
            {
                ...
                
                {
                    ...
                    sprite.MoveX(OsbEasing.InOutQuad, startTime, endTime, startX, endX + Random(-100,100));
                    sprite.MoveY(easing, startTime, endTime, startY, endY);
                    ...
                    if (FadeInDuration > 0 || FadeOutDuration > 0)
                    {
                        ...
                        sprite.Fade(easing, startTime, Math.Max(startTime, fadeInTime), 0, Color.A /dis);
                        sprite.Fade(easing, Math.Min(fadeOutTime, endTime), endTime, Color.A/ dis, 0);
                    }

                    if (StartScale != EndScale)
                        if (RandomScale)
                            sprite.Scale(easing, startTime, endTime, Random(StartScale, EndScale), Random(StartScale, EndScale));
                        else sprite.Scale(easing, startTime, endTime, StartScale *dis, StartScale *dis);
